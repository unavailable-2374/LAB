...
def createSparkConfArgs(javaOptions):
    ...
    if javaOptions is not None:
        sparkConfArgs["spark.driver.extraJavaOptions"] = sparkConfArgs[
                                                             "spark.driver.extraJavaOptions"] + ' ' + javaOptions
        sparkConfArgs["spark.executor.extraJavaOptions"] = sparkConfArgs[
                                                               "spark.executor.extraJavaOptions"] + ' ' + javaOptions
...
def getLocalGatkRunCommand(javaOptions, debugPort, debugSuspend):
    ...
    if debugPort is not None:
        if debugSuspend:
            PACKAGED_LOCAL_JAR_OPTIONS.extend(
                ["-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=" + str(debugPort)])
        else:
            PACKAGED_LOCAL_JAR_OPTIONS.extend(
                ["-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=" + str(debugPort)])
...
def getGatkWrapperScript(throwIfNotFound=True):
    if not os.path.exists(GATK_RUN_SCRIPT):
        if throwIfNotFound:
            raise GATKLaunchException(
                "Missing GATK wrapper script: " + GATK_RUN_SCRIPT + "\nTo generate the wrapper run:\n\n    " + script + "/gradlew installDist")
        else:
            return None
...
def getJarFromEnv(envVariableName):
    ...
    if jarPathFromEnv is not None:
        if not os.path.exists(jarPathFromEnv):
            raise GATKLaunchException(
                envVariableName + " was set to: " + jarPathFromEnv + " but this file doesn't exist. Please fix your environment")
        else:
...
def runGATK(sparkRunner, suppliedSparkSubmitCommand, dryrun, gatkArgs, sparkArgs, javaOptions, debugPort, debugSuspend):
    if sparkRunner is None or sparkRunner == "LOCAL":
        ...
        try:
            ...
            raise GATKLaunchException("Tried to run %s but failed.\nMake sure %s is available in your path" % (
            sparkSubmitCmd, sparkSubmitCmd))
        ...
        try:
            ...
            raise GATKLaunchException(
                "Tried to run gcloud but failed.\nMake sure gcloud is available in your path and you are properly authenticated")
    else:
        raise GATKLaunchException(
            "Value: " + sparkRunner + " is not a valid value for --spark-runner.  Choose one of LOCAL, SPARK, GCS")
